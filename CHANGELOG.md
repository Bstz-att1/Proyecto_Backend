...
### Added
- Archivos barril (`index.js`) para centralizar exportaciones por módulo:
  - `src/models/index.js`
...
### Changed
- Refactor de imports para consumir exportaciones centralizadas desde carpetas base:
  - `src/app.js`
...
- Limpieza de imports redundantes y rutas de archivo específicas, sustituidas por imports desde `../<modulo>/index.js`.
- Ajuste en `src/app.js` para declarar `PORT` y garantizar arranque estable:
  - `const PORT = process.env.PORT || 3000;`

### Notes
- Se validó arranque del backend con `cmd /c npm run dev`:
  - `nodemon` inicia correctamente.
  - Servidor levantado en `http://localhost:3000`.
- Esta versión se enfoca en mantenibilidad y legibilidad de imports sin modificar contratos de endpoints.
...
- `v1.4.3` → Normalización de respuestas en modelos de backend y mejoras de consistencia en operaciones CRUD/PATCH.
- `v1.4.4` → Ajustes de BD/datos para autenticación y fixes críticos en users controller/model.
- `v1.5.0` → Nueva capa de acceso a datos para roles y permisos efectivos por usuario.
- `v1.5.1` → Exposición HTTP inicial del módulo de roles (`GET /roles`, `GET /roles/:id`).
- `v1.5.2` → Integración de RBAC middleware y schema de gestión de roles.
- `v1.5.3` → Endurecimiento de permisos por ruta y ajuste de semilla RBAC.
- `v1.5.4` → Barrel exports por módulo, refactor de imports centralizados y fix de arranque por `PORT`.
